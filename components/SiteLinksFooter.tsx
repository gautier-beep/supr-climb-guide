  ...
  return (
    <Link
      ...
      className={`text-sm transition-colors ${
        active
          ? 'text-supr-mint font-medium'
          : 'text-black hover:text-supr-mint'
      }`}
    >
      {label}
    </Link>
...
export default function SiteLinksFooter() {
  ...
  return (
    <footer className="border-t border-supr-border bg-supr-cream mt-auto">
      <div className="max-w-2xl mx-auto px-4 py-6 pb-24 md:pb-6">
        <div className="flex items-center gap-2 mb-4">
          <Wrench className="w-4 h-4 text-supr-mint" />
        ...
        <div className="space-y-4">
          <section>
            <h3 className="text-xs font-semibold text-black mb-2">Général</h3>
          ...
          {gyms.map((gym) => (
            <section key={gym.slug}>
              <h3 className="text-xs font-semibold text-black mb-2">{gym.name}</h3>
              <ul className="flex flex-wrap gap-x-4 gap-y-2">
        ...
        <p className="text-center text-black text-[10px] mt-5">
